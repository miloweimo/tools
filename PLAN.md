...
- 实现完成后执行：`pnpm run type-check`、`pnpm exec vitest run`（与 [`ai/guides/dev-tasks.md`](ai/guides/dev-tasks.md) 约定一致）。
...
- **完整 `PLAN.md` 必须落入仓库「记录」**（与代码改动同一提交或紧随其后的提交中完成），满足可追溯、单文件可读的归档要求：
  ...
  2. **[`ai/guides/browser-fingerprint-viewer-plan.md`](ai/guides/browser-fingerprint-viewer-plan.md)**：与根目录 `PLAN.md` **内容相同**（长期有效的计划快照，符合 `ai/guides` 用途）。
...
- 可选：在 [`ai/guides/dev-tasks.md`](ai/guides/dev-tasks.md) 增加一小节里程碑勾选（与摄像头镜子条目同风格）。
